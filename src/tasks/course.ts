...
async function createCourse() {
  try {
    ...
    let lastCourse = await models.Course.findOne({
      limit: 1,
      order: [["id", "DESC"]]
    });
    if (lastCourse) {
      today = moment(lastCourse.get({ plain: true }).date, "YYYY-MM-DD").add(
        1,
        "d"
      );
    }
    ...
    while (today.toDate().getTime() < lastDay.toDate().getTime()) {
      ...
      const schedule = await models.Schedule.findOne({
        where: {
          startTime: { lte: today.startOf("day").toDate() },
          endTime: { gte: today.endOf("day").toDate() }
        }
      });
